...
//Books thar are checkedout//
// app.get("/checkout/:id/book", async function (request, respond) {
//     const checkoutbook = await Checkout.find(bookId)
// })

//Books written by author//
// app.get("/author/:id/book", async function (request, respond) {
//     const authorBook = await Author.find({ authorId: request.params.id }).lean().exec()
//     const author = await Book.findById(request.params.id);
//     return respond.status(201).json({ books: authorBook, author: author });
// })
